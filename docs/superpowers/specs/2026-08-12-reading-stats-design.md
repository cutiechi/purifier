...
```
ReadPage / BookPage
   │  useReadingSession({ site, kind, id, title })
...
建表后，若 `reading_sessions` 为空且 `items` 非空，执行一次性回填：
...
- 仅用于「活跃日密度」，不伪造时长。回填只在表行数为 0 时跑（用 `SELECT count(*)` 判空），避免重复。

### Store 新增域（对齐现有 `store.ts` 风格）

- `recordSession({ site, kind, itemId, title, startedAt, durationS })` → `INSERT`。
- `getStats({ site? })` → 聚合返回各 section（见下）。
- `exportBackup` 增加导出 `reading_sessions` 全表行。

## 时区（关键细节）

- `started_at` 存 UTC 毫秒；「日」边界以**服务端本地时区**为准（Bun 进程 TZ；SQL 用 `date(started_at / 1000, 'unixepoch', 'local time')` 取本地日期，`strftime('%H', …, 'local time')` 取本地小时）。
- **假设**：自托管个人机 / Docker 容器的 `TZ` 与用户一致（Dockerfile 文档提示设 `TZ=Asia/Shanghai` 之类）。若不一致，跨日的连读 / 热力图会错位一格。
...
校验：`kind ∈ {post, book}`、`site` 经 `resolveSite` 有效、`id` / `title` 非空、`startedAt` 正整数、`durationS` 非负整数。**`durationS < 3` 的小段直接丢弃不写**（去噪）。未知字段忽略。返回 `{ ok }`。

### `GET /api/me/stats?site=`（读，`NO_STORE_HEADERS`）

`site` 可选，默认跨站聚合；带 `site=1|2` 只算该站，过滤对所有 section 生效。返回：

```json
{
  "summary": {
    "totalDurationS": 0,       // sum(duration_s)，不含 NULL
    "currentStreak": 0,        // 连续活跃日（锚点为今天，今天无则昨天）
...
**连读定义**：取所有活跃日的本地日期集合（真实 + 回填都算），从今天（今天无活动则从昨天）往回数连续天数 = `currentStreak`；该集合的最长连续段 = `longestStreak`；集合大小 = `activeDays`。

## 前端

### `useReadingSession({ site, kind, id, title })` hook

放在 `apps/web/src/hooks/`（或 `lib/`）。只在三者齐、且非 SSR 时启动。
...
- **flush**：`durationS = round(accumulatedMs / 1000)`；若 `≥ 3`，POST `api.meSessions`，payload 含 `startedAt = segStartMs`（该段起点）、`kind/site/id/title`；之后重置 `accumulatedMs = 0`、`segStartMs = now`（visible 时）。
- 提交方式：`navigator.sendBeacon(url, blob(JSON, 'application/json'))`；不可用回退 `fetch(url, { method: 'POST', body, keepalive: true, headers: {'Content-Type':'application/json'} })`。
- 失败（配额 / 离线）：静默丢弃该段，不阻塞阅读、不补传（YAGNI）。

挂载点：`ReadPage` 与 `BookPage` 各在拿到正文上下文（`site/kind/id`）后调用一次；同一次页面停留 = 一个 hook 实例。
...
- 顶部 `PageSiteTabs`（全部 / 论坛 / 书库，默认全部）→ `site` 变化重取。
- `useEffect` fetch `api.meStats`（带可选 `site`），用 `AsyncBody` 包加载 / 错误 / 空态。
- 板块（纯展示，**不引图表库**，div / SVG 手写 + Tailwind 4）：
  1. **概览大数字**：累计时长、当前连读、最长连读、活跃天数、本周 / 本月时长、记录始于。
  2. **每日热力图**：近 365 天 GitHub 式 7×N 网格；深浅 = 当日 `durationS`（分档或对数）；`estimated` 日加斜纹 / 描边区分；`title` 显示「日期 · 时长」。
  3. **时段分布**：24 根柱（或 0–6 / 6–12 / 12–18 / 18–24 四段），高度 = 该小时时长 S；一眼看出「几点在读」。
  4. **时长 TOP**：前 10 篇，条形 + 时长；点击 `readPath` / `bookPath` 进阅读页。
  5. **最近阅读**：最近 20 条（时间 · 篇名 · 时长），可点进。
  6. **库存计数**：历史 / 收藏 / 标签 / 分组 / 角色五个小数字卡（取自 `inventory`）。
- `lib/format.ts` 增 `formatDuration(s)`（如 `3720 → "1h 2m"`、`90 → "1m 30s"`）。

## 边界与错误处理

- **路由切换**：旧 hook 卸载 flush + 新 hook 挂载。React StrictMode dev 双挂载会多记一小段，仅 dev、生产无影响（接受）。
- **跨天阅读**（23:50 读到 00:10）：按段拆分，各段 `startedAt` 落在各自日期，天然正确。
- **极短停留**（<3s）：丢弃，不写。
- **sendBeacon 失败 / 离线**：静默丢该段，不阻塞、不补传。
- **零数据首跑**：`AsyncBody` 空态；`inventory` + `trackedSince` 仍给页面内容。

## 测试（`packages/core`，`bun test`）

- `recordSession`：正常插入、`<3s` 不写、字段校验（真实采集路径 `durationS` 恒 ≥ 3；NULL 只由回填迁移产生，不走此方法）。
- `getStats`：用固定时间戳构造一批 sessions，断言：`summary`（`totalDurationS` 忽略 NULL、`currentStreak` / `longestStreak` / `activeDays`）、`calendar` 按本地日分桶、`timeOfDay` 按本地小时、`topItems` 排序、`site` 过滤生效。
